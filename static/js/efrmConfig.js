...
angular.module('efrm.config')
	...
	.factory('RequestsErrorHandler', ['$q', '$injector','$rootScope', 'Msg', '$state','$timeout','commonDataService', function($q,  $injector, $rootScope, Msg, $state, $timeout,commonDataService) {
		var toastr;
		var counter = 0;
		...
		return {
            ...
            response: function (response) {
            	//new cahnges start
            	if(typeof commonDataService.getLocalStorage() != 'undefined'){
	            	var myurl = response.config.url;
	            	myurl = myurl.substring(0, myurl.indexOf('?'));
	            	
	            	if(myurl=="config/bankdetails/v1/channels" && commonDataService.getLocalStorage().orgId != 'NPCI'){
	            		
	                    for(var i = response.data.response.length - 1; i >= 0; i--) {
	                        if(response.data.response[i].channelCode === 'AEPS') {
	                        	response.data.response.splice(i, 1);
	                        }
	                    }
