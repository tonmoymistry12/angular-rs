    ...
    return {
      ...
      link: function(scope, element, attrs, ctrl) {
        ...
        scope.$watch(attrs.ngModel, function(value) {
          ...
          console.log(attrs.fromDate)
          // The value has already been cleaned by the above code
          var date = new Date(value);
          ctrl.$setValidity('date', !date? false : true);
          var now = new Date();
          var futureOn = new Date(attrs.fromDate);
          var pastOn = new Date(attrs.pastDate);
          if( attrs.hasOwnProperty('futureOn') ){
              ctrl.$setValidity('future-on', date < futureOn? false : true);
          }
          if( attrs.hasOwnProperty('pastOn') ){
              ctrl.$setValidity('past-on', date > pastOn? false : true);
          }
