            ...
            controller: function(Idle,Keepalive,$scope, $ngConfirm, $window, $state, $location, $http, ruleEditorManagement, toastr, Msg, casesManagement2, editPermission, RuleService, ruleManagement,ruleDataService,commonDataService,ngDialog, $timeout, $filter) {
            	Idle.watch();
				  ...
				  $scope.$on('IdleStart', function() {
					  ...
					  ngDialog.open({ template: 'sessionTimeOut', scope: $scope });
				  });
             ...
             //   var perspective = localStorage.getItem("perspective");
              ...
              //  $scope.formParams.ruleMetaData.userInformationDTO.orgId=orgId;
               ...
               /* $scope.dateChange=function(date){
                ...
                $scope.$watch('formParams.ruleMetaData.orgId', function (newValue, oldValue, scope) {
                	
                	if($scope.formParams.ruleMetaData.orgId!=undefined){
                		
                		if($scope.orgId==='NPCI'){
                			if(!$scope.formParams.ruleMetaData.orgId.contains("NPCI")){
                				$scope.formParams.ruleMetaData.aml=false
                				$scope.chkAml=false;
                				$scope.disableSubmit=false;
                			}else{
                				$scope.formParams.ruleMetaData.aml=true
                				if($scope.formParams.ruleMetaData.orgId.split(",").length>=1 && $scope.formParams.ruleMetaData.orgId==='NPCI'){
                					$scope.disableSubmit=false;
                					$scope.chkAml=true;
                				}else{
                					$scope.chkAml=false;
                				...
                				if($scope.formParams.ruleMetaData.orgId.split(",").length>1){
                ...
                $scope.getRule = function() {
                   ...
                   // alert(RuleService.getRules().ruleDetailsId)
                    ...
                    ruleEditorManagement.header({}).getRule({
                     ...
                     //var viewservice = RuleService.getCopyFlag()
                        ...
                        $scope.getRuleData = data.response
                   //   console.log('getRULE from API - ',$scope.getRuleData);
                    ...
                    //	console.log('GET - ',$scope.getRuleData)
                     ...
                     // console.log('Edit OrgID ',$scope.formParams.ruleMetaData.userInformationDTO.orgId)
                     // console.log('Edit userId ',$scope.formParams.ruleMetaData.userInformationDTO.userId)
